...
- 参与者：已注册用户（遗忘密码）
- 前置条件：账户已绑定并通过校验的手机号码；账户与证件信息一致；系统可用
- 主成功场景（四步流程）：
  1. 第一步 填写账户信息：用户选择国家码（默认 +86），输入手机号码，选择证件类型并填写证件号码，点击“提交”进入下一步
  2. 第二步 获取验证码：页面展示手机号；用户点击“获取手机验证码”，收到短信后在输入框填写验证码，点击“提交”进入下一步
  3. 第三步 设置新密码：输入新密码与确认密码（需满足密码策略），点击“提交”
  4. 第四步 完成：页面提示“新密码设置成功，您可以使用新密码登录系统！”，提供“登录系统”链接
- 备选/异常流：
  - 手机号码未通过校验/证件不匹配 → 阻止进入下一步并提示
  - 验证码错误/过期 → 阻止提交并提示，可重发（限制频率与次数）
  - 新密码不符合策略/两次输入不一致 → 阻止提交并提示
  - 多次错误达到上限 → 触发暂时性保护（冷却时间/二次校验）
- 后置条件：账户密码更新；旧会话与重置令牌失效；可用新密码登录
- 验收点：四步页面流程与提示准确；完成页提供登录入口
...
- F 用户功能总览
  - F1 用户认证
    ...
    - F1.3 忘记密码
      - F1.3.1 手机找回（四步）
        - F1.3.1.1 填写账户信息（手机、证件类型、证件号）
        - F1.3.1.2 获取验证码（发送/输入/提交）
        - F1.3.1.3 设置新密码（策略校验与确认）
        - F1.3.1.4 完成（成功提示与登录入口）
      - F1.3.2 邮箱找回（发送邮件链接/验证码，待规划）
  - F2 常用乘车人
    - F2.1 查看（列表展示）
...
- N 非功能总览
  - N1 安全
    ...
    - N1.5 忘记密码保护
      - 验证码策略：长度 6、TTL 5 分钟、最多重试 5 次、发送频率限制（如 60 秒/次，日上限）
      - 渠道校验：仅允许绑定并通过校验的手机/邮箱
      - 密码策略：不少于 6 位，需至少包含字母/数字/下划线中的两种；拒绝弱口令
      - 令牌失效：密码重置成功后，旧令牌和旧会话失效
      - 防暴力：错误次数达到阈值后短时锁定或二次验证
  - N2 性能
    - N2.1 查询响应 ≤ 2s
    - N2.2 列表/筛选 UI 反馈 ≤ 1s
  - N3 可靠性
...
- D 数据总览
  - D1 表结构
    ...
    - D1.6 password_resets / verification_codes（忘记密码与验证码）
      - 字段示例：`id/user_id/channel(phone|email)/code/expires_at/attempts/status(created|used|expired)`
      - 说明：支持同一用户在有效期内唯一有效验证码；状态流转为 used/expired
  - D2 约束与索引
...
- test8-1 忘记密码－手机找回－第一步（账户信息校验与提交） → S8
- test8-2 忘记密码－手机找回－第二步（验证码发送/输入/提交） → S8
- test8-3 忘记密码－手机找回－第三步（新密码与确认校验） → S8
- test8-4 忘记密码－手机找回－第四步（成功提示与登录入口） → S8
...
- S8：忘记密码（手机找回）
  - 第一步：手机号、证件类型与证件号校验正确；可提交进入第二步
  - 第二步：验证码发送受限于频率与次数；输入正确可提交进入第三步
  - 第三步：密码策略与确认一致性校验通过；可提交进入完成页
  - 第四步：显示“新密码设置成功，您可以使用新密码登录系统！”，提供“登录系统”链接
...
---
  ...
  - 规划中 F1.3 忘记密码（手机找回）
    - 四步流程与页面说明已纳入本需求文档；后端验证码与密码重置接口将在安全策略（N1.5）与数据结构（D1.6）落地后实现
    - 前端路由占位：`/forgot-password`、`/forgot-password/verify`、`/forgot-password/reset`、`/forgot-password/done`（用于 UI 验收与演示）
