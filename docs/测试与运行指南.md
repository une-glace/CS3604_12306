...
- 启动服务：
  ...
  - 可选：通过环境变量覆盖 E2E 访问地址 `PW_BASE_URL`（默认为 `http://localhost:5174`）。例如：`PW_BASE_URL=http://localhost:5175 npm run test:e2e --prefix frontend`
...
**注册与登录口径（统一于前后端与测试）**
- 用户名：以字母开头，允许字母/数字/下划线，长度 6-30（`backend/src/models/User.js:15-17`、`backend/src/utils/validation.js:7-13`）
- 密码：长度 ≥ 6（后端）；前端页面交互建议至少包含两类字符以避免弱密码（测试使用示例 `my_password1`）（`backend/src/utils/validation.js:24-28`、`frontend/tests/e2e/utils/auth.ts:3-5`）
- 手机号：支持中国大陆或通用 E.164（`backend/src/utils/validation.js:80-90`、`backend/src/models/User.js:59-61`）
- 证件号：居民身份证 18 位，含合法校验码（后端在生产环境严格校验；开发/测试环境放宽注册）`backend/src/utils/validation.js:56-76`

**E2E 登录稳定性策略**
- 统一使用 `ensureLogin(page)`（`frontend/tests/e2e/utils/auth.ts:3-58`）：优先接口登录，必要时注册并设置 `localStorage`，随后刷新站点与进入个人中心，避免跨域页面上下文导致的 `localStorage` 读写异常。
